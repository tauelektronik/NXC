		...
		if (self::$db->num_rows() > 0) {
			...
			if (self::$db->num_rows() > 0) {
				...
				if ($rStream['server_info']['on_demand'] == 1) {
					...
					// FIX #80: use stream_max_analyze from panel settings instead of hardcoded 10s
					// hardcoded '10000000' caused 20-30s startup delay on M3U8 on-demand streams
					$rAnalyseDuration = abs(intval(self::$rSettings['stream_max_analyze'])) ?: 2000000;
				} else {
					$rAnalyseDuration = abs(intval(self::$rSettings['stream_max_analyze']));
					$rProbesize = abs(intval(self::$rSettings['probesize']));
				}
